...
[[ -f "$project_dir/native/MermaidStudioViewer/Package.swift" ]] && printf 'Swift project: %s\n' "$(command -v swift || echo 'swift missing')"
...
if $build; then
  ...
  [[ -f native/MermaidStudioViewer/Package.swift ]] && swift build --package-path native/MermaidStudioViewer
fi

if $test_project; then
  confirm 'Run the available local test commands?' || exit 0
  [[ -f package.json ]] && npm run test:release --if-present
  [[ -f native/MermaidStudioViewer/Package.swift ]] && npm run test:native --if-present
